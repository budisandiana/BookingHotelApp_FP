    ...
    def add_entry(self, lname, fname, number, checkin, checkout, availability):
        ...
        for i in range(len(self.rez.lcamera)):
            if self.rez.lcamera[i] == number:
                if int(self.rez.lcheckout[i][-4:]) == int(checkin[-4:]) and int(self.rez.lcheckout[i][:2]) == int(checkin[:2]):
...
class FilterByPrice:
    ...
    def search(self, price):
        """
        Se cauta camere cu pretul mai mic decat entry_price
...
class FilterByView:
    ...
    def search(self, seaview):
        """
        Se cauta entry_view in lista cu views
        :param seaview: entry_view
        """
...
class FilterByBoth:
    ...
    def search(self, price, seaview):
        """
        Se cauta entry_price in lista cu preturi si entry_view in lista cu views
        :param price: entry_price
        :param seaview: entry_view
        """
        self.price = price
